...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let upstream_proto = Path::new("../pookiepy/message.proto");
    let vendored_proto = Path::new("pookiepy/message.proto");

    let (proto_file, project_include): (&str, PathBuf) = if upstream_proto.exists() {
        ("../pookiepy/message.proto", PathBuf::from(".."))
    } else if vendored_proto.exists() {
        ("pookiepy/message.proto", PathBuf::from("."))
    } else {
        return Err(
            "message.proto not found; expected ../pookiepy/message.proto or pookiepy/message.proto"
                .into(),
        );
    };
    ...
    println!("cargo:rerun-if-changed=../pookiepy/message.proto");
    println!("cargo:rerun-if-changed=pookiepy/message.proto");
