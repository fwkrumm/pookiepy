...
class MyServer(BaseServer):
    ...
    def on_data_yield(self, peer: Peer, data: message_pb2.PookieMessage):
        # just before server-side yield to client stream
        # telemetry/metrics hook; not delivery confirmation
        pass

    def _handle_request(self, peer: Peer, request: message_pb2.PookieMessage):
...
class MyClient(BaseClient):
    ...
    def on_data_yield(self, data: message_pb2.PookieMessage):
...
|---|---|---|
| `send_data` | `(msg: PookieMessage, add_history=False)` | Enqueue send. `messageName` must be in `provides`; `add_history` appends first `DataPoint`. |
| `get_data` | `(timeout=None) → PookieMessage` | Poll receive queue. `None`=wait forever, `0`=non-blocking. |
...
PookieMessages may carry per-hop `history`. Use `add_history=True` on send to append first hop automatically. Use `evaluate_history(data, log_callback)` later for per-hop latency.
...
1. Keep the required `PookieMessage`, metadata, history, client/server information, and bidirectional `Stream.DataChannel` definitions.
